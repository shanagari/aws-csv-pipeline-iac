...
- `sample-data/movies.csv`, `sample-data/ratings.csv` — test data
...
is the separate step that loads `sample-data/movies.csv` and
`sample-data/ratings.csv` into the input bucket, which is what actually
fires the pipeline.
...
- **Automatic**: push a change to `sample-data/movies.csv` or
  `sample-data/ratings.csv` — `.github/workflows/upload-data.yml` runs
  automatically.
- **Manual**: **Actions tab → Upload Sample Data → Run workflow**.

This looks up the input bucket from the deployed stack and uploads
`movies.csv` then `ratings.csv` — see the trigger note above for why the
order matters.
